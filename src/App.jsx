...
import { MapPin, DollarSign, Home, Key, TrendingUp, Eye, Star, Camera, Landmark } from 'lucide-react';
...
export default function App() {
  ...
  // Function to get station income and update state
  const getStationIncome = (stationName) => {
    // Trim whitespace and normalize to handle different inputs
  ...
  const calculateRanks = () => {
    ...
    // Filter out items with 0 points (not entered) for the average calculation
  ...
  useEffect(() => {
    calculateRanks();
  }, [interior, walkToStation, rent, initialCost, floor, location, visibility, stationIncome, household1km, household1_5km]);

  // Handle nearest station input change to automatically get income data
  useEffect(() => {
    if (nearestStation) {
      getStationIncome(nearestStation);
    } else {
  ...
  const handleSaveImage = async () => {
    ...
    if (captureRef.current) {
      try {
        const canvas = await window.html2canvas(captureRef.current, { scale: 2 });
    ...
    <div className="flex flex-col items-center justify-center p-4 min-h-screen bg-gray-100 font-inter">
      <div className="w-full max-w-2xl bg-white p-6 rounded-3xl shadow-xl">
        ...
        <div className="flex flex-col items-center mt-8">
          <div className="flex items-center space-x-4 p-4 bg-gray-200 rounded-2xl w-full max-w-sm mb-6 shadow-inner">
            <Star size={24} className="text-gray-600" />
            <span className="text-lg font-bold text-gray-700">総合ランク:</span>
            <span className={`px-4 py-2 text-xl font-extrabold text-white rounded-full ${overallRank === 'N/A' ? 'bg-gray-400' : rankColors[overallRank]}`}>
              {overallRank}
            </span>
          </div>
      ...
      <div ref={captureRef} className="absolute left-[-9999px] top-[-9999px] p-6 bg-white rounded-3xl w-[600px] font-inter">
        <h2 className="text-2xl font-bold text-center mb-6 text-gray-800">物件評価レポート</h2>
        
        <div className="grid grid-cols-2 gap-2 mb-6">
          {images.map((img, index) => (
            <img key={index} src={img} alt={`物件画像 ${index + 1}`} className="w-full h-auto rounded-xl object-cover border border-gray-200" />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 mb-6">
          ...
          <div className="flex items-center justify-between p-4 bg-gray-100 rounded-xl">
            <span className="text-gray-700">最寄り駅:</span>
            <span className="text-gray-900 font-medium">
              {nearestStation || '-'}
            </span>
          </div>
          <div className="flex items-center justify-between p-4 bg-gray-100 rounded-xl">
            <span className="text-gray-700">駅周辺世帯年収:</span>
            <span className="text-gray-900 font-medium">
              {stationIncome ? `${stationIncome.toLocaleString()}円` : 'データなし'}
            </span>
            {stationIncome !== null && (
              <span className={`px-3 py-1 text-white font-bold rounded-full ${rankColors[pointToRank[rankingCriteria.income(stationIncome)]]}`}>
                {pointToRank[rankingCriteria.income(stationIncome)]}
              </span>
            )}
          </div>
          <div className="flex items-center justify-between p-4 bg-gray-100 rounded-xl">
            <span className="text-gray-700">駅徒歩:</span>
            <span className={`px-3 py-1 text-white font-bold rounded-full ${rankColors[pointToRank[rankingCriteria.walkToStation(walkToStation)]]}`}>
              {walkToStation ? `${walkToStation}分` : '-'}
            </span>
          </div>
          <div className="flex items-center justify-between p-4 bg-gray-100 rounded-xl">
            <span className="text-gray-700">賃料:</span>
            <span className={`px-3 py-1 text-white font-bold rounded-full ${rankColors[pointToRank[rankingCriteria.rent(rent)]]}`}>
              {rent ? `${parseInt(rent).toLocaleString()}円` : '-'}
            </span>
          </div>
          <div className="flex items-center justify-between p-4 bg-gray-100 rounded-xl">
            <span className="text-gray-700">初期費用:</span>
            <span className={`px-3 py-1 text-white font-bold rounded-full ${rankColors[pointToRank[rankingCriteria.initialCost(initialCost)]]}`}>
              {initialCost ? `${parseInt(initialCost).toLocaleString()}円` : '-'}
            </span>
          </div>
          <div className="flex items-center justify-between p-4 bg-gray-100 rounded-xl">
            <span className="text-gray-700">フロア:</span>
            <span className={`px-3 py-1 text-white font-bold rounded-full ${rankColors[pointToRank[rankingCriteria.floor(floor)]]}`}>
              {floor || '-'}
            </span>
          </div>
          <div className="flex items-center justify-between p-4 bg-gray-100 rounded-xl">
            <span className="text-gray-700">内装状態:</span>
            <span className={`px-3 py-1 text-white font-bold rounded-full ${rankColors[pointToRank[rankingCriteria.interior[interior]]]}`}>
              {interior || '-'}
            </span>
          </div>
          <div className="flex items-center justify-between p-4 bg-gray-100 rounded-xl">
            <span className="text-gray-700">立地:</span>
            <span className={`px-3 py-1 text-white font-bold rounded-full ${rankColors[pointToRank[rankingCriteria.location[location]]]}`}>
              {location || '-'}
            </span>
          </div>
          <div className="flex items-center justify-between p-4 bg-gray-100 rounded-xl">
            <span className="text-gray-700">視認性:</span>
            <span className={`px-3 py-1 text-white font-bold rounded-full ${rankColors[pointToRank[rankingCriteria.visibility[visibility]]]}`}>
              {visibility || '-'}
            </span>
          </div>
          <div className="flex items-center justify-between p-4 bg-gray-100 rounded-xl">
            <span className="text-gray-700">周辺世帯数 (1km):</span>
            <span className="text-gray-900 font-medium">
              {household1km ? `${parseInt(household1km).toLocaleString()}世帯` : '-'}
            </span>
            {household1km && (
              <span className={`px-3 py-1 text-white font-bold rounded-full ${rankColors[pointToRank[rankingCriteria.household1km(household1km)]]}`}>
                {pointToRank[rankingCriteria.household1km(household1km)]}
              </span>
            )}
          </div>
          <div className="flex items-center justify-between p-4 bg-gray-100 rounded-xl">
            <span className="text-gray-700">周辺世帯数 (1.5km):</span>
            <span className="text-gray-900 font-medium">
              {household1_5km ? `${parseInt(household1_5km).toLocaleString()}世帯` : '-'}
            </span>
            {household1_5km && (
              <span className={`px-3 py-1 text-white font-bold rounded-full ${rankColors[pointToRank[rankingCriteria.household1_5km(household1_5km)]]}`}>
                {pointToRank[rankingCriteria.household1_5km(household1_5km)]}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center justify-center p-4 bg-blue-500 rounded-xl mt-4">
          <span className="text-white text-lg font-bold mr-4">総合ランク:</span>
          <span className={`px-4 py-2 text-2xl font-extrabold text-white rounded-full bg-black bg-opacity-20`}>
            {overallRank}
          </span>
        </div>
      </div>
